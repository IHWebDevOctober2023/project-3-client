    ...
    const putHelp = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_ROOT}/help-post/${helpId}`, {
                ...
                body: JSON.stringify(helpPut),
            }
                , { mode: 'cors' });
            const newHelpPut = await response.json();
            setHelpPut((previousHelpPut) => [newHelpPut, ...previousHelpPut])
            console.log("helpPut",helpPut);
        ...
        <div>
            ...
            <div className="edit-help-container">
                <form className="create-help-container" onSubmit={(event) => putHelp(event)}>
                    ...
                    <p className="create-help-button" type="submit">SAVE CHANGES</p>
                </form>
            </div>
